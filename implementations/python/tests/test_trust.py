...
from mzprov.errors import ProvenanceError
from mzprov.keys import (
    derive_key_id,
    generate_keypair,
    write_keypair,
)
from mzprov.trust import (
    REGISTRY_SCHEMA,
    TrustedKey,
    TrustedKeyRegistry,
...
def test_expected_key_id_cannot_be_bypassed_by_payload_relabel(tmp_path):
    ...
    """
    import base64
    from mzprov.envelope import Payload
    from mzprov.errors import MalformedSidecar
...
def test_expected_key_id_uses_derived_signer_not_payload_field(tmp_path):
    ...
    """
    from mzprov.keys import derive_key_id
...
def test_keys_cli_show(tmp_path, capsys):
    from mzprov.keys_cli import main as keys_main
...
def test_keys_cli_untrust_missing_id_returns_4(tmp_path, capsys):
    from mzprov.keys_cli import main as keys_main, EXIT_KEY_NOT_FOUND
    reg_path = tmp_path / "trusted.json"
    rc = keys_main([
...
def test_verify_cli_expected_key_id_match(tmp_path):
    from mzprov.cli import main as verify_main, EXIT_OK
    _, _, _, sidecar, keypair = _build_signed_bundle_with_keypair(tmp_path)
    rc = verify_main([
        str(sidecar.parent),
